...
_html2canvas.global.Log = function(a) {
...
_html2canvas.Util.Children = function( elem ) {
  ...
  try {
    ...
    _html2canvas.global.Log("html2canvas.Util.Children failed with exception: " + ex.message);
    children = [];
  }
  return children;
};
  ...
  function addScrollStops(grad) {
    return function(colorStop) {
      try {
        ...
        global.Log(['failed to add color stop: ', e, '; tried to add: ', colorStop]);
      }
    };
  }
...
_html2canvas.Parse = function (images, options, cb) {
  ...
  function init() {
    ...
    parseChildren(element, stack, function() {
      ...
      global.Log('Done parsing, moving to Render.');
  ...
  function renderFormValue (el, bounds, stack){
    ...
    cssPropertyArray.forEach(function(property) {
      try {
        ...
        global.Log("html2canvas: Parse: Exception caught in renderFormValue: " + e.message);
      }
    });
  ...
  function getPseudoElement(el, which) {
    ...
    Object.keys(elStyle).filter(indexedProperty).forEach(function(prop) {
      ...
      try {
        ...
        global.Log(['Tried to assign readonly property ', prop, 'Error:', e]);
      }
    });
  ...
  function renderBackgroundImage(element, bounds, ctx) {
    ...
    while(imageIndex--) {
      ...
      if (image) {
        ...
        global.Log("html2canvas: Error loading background:", backgroundImage);
      }
    }
  }
    ...
    switch(element.nodeName){
      case "IMG":
        if ((image = loadImage(element.getAttribute('src')))) {
          ...
          global.Log("html2canvas: Error loading <img>:" + element.getAttribute('src'));
        }
        break;
      case "INPUT":
  ...
  function parseChildren(element, stack, cb) {
    ...
    function finished(el) {
      if (--jobs <= 0){
        global.Log("finished rendering " + children.length + " children.");
        cb();
      }
    }
  }
...
_html2canvas.Preload = function( options ) {
  ...
  function start(){
    global.Log("html2canvas: start: images: " + images.numLoaded + " / " + images.numTotal + " (failed: " + images.numFailed + ")");
    if (!images.firstRun && images.numLoaded >= images.numTotal){
      global.Log("Finished loading images: # " + images.numTotal + " (failed: " + images.numFailed + ")");
  ...
  function getImages (el) {
    ...
    try {
      ...
      elNodeType = false;
      global.Log("html2canvas: failed to access some element's nodeType - Exception: " + ex.message);
    }
    ...
    if (elNodeType === 1 || elNodeType === undefined) {
      ...
      try {
        ...
        global.Log("html2canvas: failed to get background-image - Exception: " + e.message);
      }
    ...
    cleanupDOM: function(cause) {
      ...
      if (!images.cleanupDone) {
        if (cause && typeof cause === "string") {
          global.Log("html2canvas: Cleanup because: " + cause);
        } else {
          global.Log("html2canvas: Cleanup after timeout: " + options.timeout + " ms.");
        }
        ...
        for (src in images) {
          if (images.hasOwnProperty(src)) {
            ...
            if (typeof img === "object" && img.callbackname && img.succeeded === undefined) {
              ...
              images.numLoaded++;
              images.numFailed++;
              global.Log("html2canvas: Cleaned up failed img: '" + src + "' Steps: " + images.numLoaded + " / " + images.numTotal);
            }
          }
        }
  ...
  global.Log('html2canvas: Preload starts: finding background-images');
  images.firstRun = true;

  getImages(element);

  global.Log('html2canvas: Preload: Finding images');
  // load <img> images
  ...
  global.Log('html2canvas: Preload: Done.');
...
_html2canvas.Util.Support = function (options, doc) {
  ...
  function supportSVGRendering() {
    ...
    _html2canvas.global.Log('html2canvas: Parse: SVG powered rendering available');
    return true;
  }
...
window.html2canvas = function(elements, opts) {
  ...
  return {
    ...
    log: _html2canvas.global.Log
  };
};

window.html2canvas.log = _html2canvas.global.Log; // for renderers
...
_html2canvas.Renderer.Canvas = function(options) {
          ...
          case "createPattern":
            if (item['arguments'][0].width > 0 && item['arguments'][0].height > 0) {
              try {
                ...
                global.Log("html2canvas: Renderer: Error creating pattern", e.message);
              }
            }
            break;
          case "drawShape":
  ...
  return function(parsedData, options, document, queue, _html2canvas) {
    ...
    global.Log("html2canvas: Renderer: Canvas renderer done - returning canvas obj");
