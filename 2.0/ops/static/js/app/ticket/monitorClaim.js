...
$(document).ready(function () {
    ...
    vm = new Vue({
        ...
        methods: {
             ...
             * @param orderid
             */
            setPrizeRefresh: function (orderid) {
                commonSwal("是否设置重新兑奖", '', function () {
                    post(params.PrizeSetUrl, {
                        "opt": 1,
                        "orderid":  orderid,
                    }, function (data) {
                        if (data && data.code == Constant.SUCCESS) {
             ...
             * @param orderid
             */
            setPrizeOk: function (orderid) {
                commonSwal("是否标记已兑奖", '', function () {
                    post(params.PrizeSetUrl, {
                        "opt": 2,
                        "orderid":  orderid,
                    }, function (data) {
    ...
    function dataRefresh() {
        ...
        mypost(params.prizeMonitorListUrl, {}, function (data) {
            ...
            $('#AssignFlag-button').text(vm.mydata.assignswitch?'关闭分发兑奖':'打开分发兑奖')
            if (data) {
                setTimeout(function () {
                    if (vm.flashOnOrOff) {
                        vm.fetchDashboard();
